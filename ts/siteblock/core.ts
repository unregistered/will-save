...
export class DatastoreAccess {
  ...
  incrementCurrency(times: number, onComplete: (newLevel: number) => void) {
    this.getDefaultCurrencyPerLesson(gain => {
      this.store.getData<number>(
        DataKey.CURRENCY_COUNT,
        currency => {
